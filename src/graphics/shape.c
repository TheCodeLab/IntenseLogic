...
ilG_shape *ilG_box(ilG_context *context)
{
    ...
    ilG_testError("Unable to upload cube data");
    ...
    ilG_testError("Unable to set vertex attrib pointer");
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(cube_index), cube_index, GL_STATIC_DRAW);
    ilG_testError("Unable to upload index buffer data");
...
void ilG_shape_draw(ilG_shape *shape)
{
    ...
    ilG_testError("Could not draw shape");
}
