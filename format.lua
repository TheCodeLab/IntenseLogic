...
-- vars
local res = io.popen('find . -name "*.inc" -type f')
...
for _,v in ipairs(target) do
    local s = FS.readfile(v)
    s = s:gsub("il_(%w+)_?(%w*)_?(%w*)", function(a, b, c)
        if b ~= "" then
            local mod = a:sub(1,1):upper()

            if mod == "C" then 
                mod = ""
            end

            return "il" .. mod .. "_" .. (b:sub(1,1):lower() .. b:sub(2)) .. (c ~= "" and "_" or "") .. c
        else
            return "il_" .. a
        end
    end)
